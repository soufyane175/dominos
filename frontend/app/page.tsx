...
export default function Home() {
  ...
  // ✅ Socket verbinding met JOUW Render server
  useEffect(() => {
    console.log("🔌 Verbinden met:", SOCKET_URL);
    ...
    s.on("connect", () => {
      setStatus("on");
      console.log("✅ Verbonden met server! ID:", s.id);
    });
    ...
    s.on("disconnect", (reason) => {
      setStatus("off");
      console.log("🔌 Verbroken:", reason);
    });
    s.on("reconnect", () => {
      setStatus("on");
      console.log("🔄 Opnieuw verbonden!");
    });

    // Room events
    s.on("roomCreated", ({ code }: any) => {
      console.log("🏠 Kamer gemaakt:", code);
    ...
    s.on("gameStarted", ({ p1, p2 }: any) => {
      console.log("🎮 Game start:", p1, "vs", p2);
      setWaiting(false);
      sys(`🎮 ${p1} vs ${p2} - LET'S GO!`);
    });

    s.on("gameState", (data: any) => {
      console.log("📊 Game state update, turn:", data.turn, "myIdx:", data.playerIndex);
      setBoard(data.board);
      setHand(data.hand);
    ...
    s.on("opponentLeft", () => { sys("❌ Tegenstander heeft het spel verlaten!"); setGameOver("❌ Tegenstander weg"); });
    s.on("gameRestarted", () => { setGameOver(null); sys("🔄 Nieuw spel gestart!"); });
  ...
  const oCreate = () => {
    if (status !== "on") {
      setJoinErr("Server wordt opgestart... Wacht 30 sec en probeer opnieuw!");
      // Render free tier slaapt - wake up
  ...
  const oJoin = () => {
    ...
    if (c.length < 4) { setJoinErr("Code moet 5 tekens zijn!"); return; }
    if (status !== "on") {
      setJoinErr("Server wordt opgestart... Wacht 30 sec!");
      fetch(SOCKET_URL).catch(() => { });
      return;
    }
    setJoinErr("");
  ...
  const startBot = () => {
    ...
    setTimeout(() => sys("🤖 Jij begint! Klik op een steen."), 100);
  };
  ...
  const bPlay = (tile: Tile, idx: number) => {
    ...
    if (!side && board.length > 0) { sys("❌ Past niet! Kies een andere of pak uit de pot."); return; }
    let fl = false;
    ...
    setBotHand(prev => {
      ...
      if (pi === -1) {
        setPile(pp => {
          ...
          sys("🤖 Bot pakt uit de pot."); setBotHand(bh => [...bh, d]); setMyTurn(true);
      ...
      setBoard(nb2); sys(`🤖 Bot speelde [${bt[0]}|${bt[1]}]`);
      const nbh = prev.filter((_, i) => i !== pi);
      if (!nbh.length) { setGameOver("💀 BOT WINT!"); sys("💀 Bot heeft gewonnen!"); }
  ...
  const bDraw = () => {
    ...
    sys(`📦 Je pakte [${d[0]}|${d[1]}]`);
  };

  const bPass = () => {
    if (!myTurn || gameOver) return;
    setMyTurn(false); sys("⏭️ Je hebt gepast.");
  ...
  const bChat = (t: string) => {
    ...
    setTimeout(() => {
      const r = ["😂", "Nice 🔥", "GG 💯", "💪", "Biji ☀️", "👏", "🤖💪", "👍", "Haha", "Ik win! 😈", "Goed gespeeld!", "☀️🎲"];
  ...
  return (
    <main className="min-h-[100dvh] flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="fixed top-2 right-2 flex gap-1 z-50 items-center">
        {view === "menu" && (
          <div className="flex items-center gap-1 bg-black/50 px-2 py-0.5 rounded-full mr-1 backdrop-blur-sm">
            <div className={`w-2 h-2 rounded-full ${status === "on" ? "bg-green-400" : status === "err" ? "bg-red-500" : "bg-gray-500"} ${status === "on" ? "animate-pulse" : ""}`} />
            <span className="text-white/50 text-[9px] font-medium">
              {status === "on" ? "Online ✓" : status === "err" ? "Herverbinden..." : "Verbinden..."}
            </span>
          </div>
        )}
        <button onClick={() => setLang("KU")} className={`px-2 py-0.5 rounded-full font-bold text-[10px] transition-colors ${lang === "KU" ? "bg-yellow-500 text-black" : "bg-black/40 text-white/60"}`}>KU</button>
        <button onClick={() => setLang("NL")} className={`px-2 py-0.5 rounded-full font-bold text-[10px] transition-colors ${lang === "NL" ? "bg-yellow-500 text-black" : "bg-black/40 text-white/60"}`}>NL</button>
      </div>
      ...
      {view === "menu" ? (
        <div className="flex flex-col items-center z-10 w-full max-w-[300px] px-5 py-4">
          ...
          <p className="text-white/80 text-[10px] mb-4 font-bold bg-black/30 px-3 py-1 rounded-full backdrop-blur-sm">☀️ 28 stenen • 7 per speler</p>
          ...
          <button onClick={startBot} className="w-full bg-gradient-to-b from-blue-500 to-blue-700 text-white font-black py-3 rounded-2xl border-b-4 border-blue-900 mb-2 active:translate-y-0.5 active:border-b-2 shadow-xl text-sm transition-all">
            🤖 {lang === "KU" ? "یاری دژی بۆت" : "Tegen Bot"}
          </button>

          <button onClick={oCreate} className="w-full bg-gradient-to-b from-green-500 to-green-700 text-white font-black py-3 rounded-2xl border-b-4 border-green-900 mb-2 active:translate-y-0.5 active:border-b-2 shadow-xl text-sm transition-all">
          ...
          <div className="bg-gradient-to-b from-purple-500 to-purple-700 p-3 rounded-2xl border-b-4 border-purple-900 w-full shadow-xl">
            <input
              ...
              className="w-full p-2.5 rounded-xl text-center font-bold mb-1.5 uppercase text-black bg-white border-2 border-purple-300 text-lg tracking-[0.3em] focus:outline-none focus:ring-2 focus:ring-purple-400"
              maxLength={5}
            />
            {joinErr && (
              <p className="text-white text-[10px] text-center mb-1.5 font-bold bg-red-500/30 rounded-lg py-1 px-2">
                ⚠️ {joinErr}
              </p>
            )}
            <button onClick={oJoin} className="w-full text-white font-black py-2.5 bg-purple-900/50 rounded-xl active:bg-purple-800 text-sm transition-colors">
          ...
          {status !== "on" && (
            <div className="mt-3 bg-yellow-500/20 rounded-xl p-2 w-full">
              <p className="text-yellow-300 text-[10px] text-center font-bold">
                ⏳ {lang === "KU" ? "سێرڤەر لە خەوە... ١٥ چرکە چاوەڕوان بە" : "Server start op... wacht ~15 seconden"}
              </p>
              <button onClick={() => { fetch(SOCKET_URL).catch(() => { }); setJoinErr(""); }}
                className="w-full mt-1 text-yellow-400 text-[10px] font-bold bg-yellow-500/10 rounded py-1 active:bg-yellow-500/20">
                🔄 {lang === "KU" ? "هەوڵ بدەوە" : "Opnieuw proberen"}
              </button>
            </div>
          )}
        ...
        <div className="w-full h-[100dvh] flex flex-col z-10">
          ...
          {/* Top bar */}
          <div className="flex items-center justify-between px-2 py-1.5 bg-black/60 backdrop-blur-sm flex-shrink-0">
            ...
            <div className="flex items-center gap-1.5">
              ...
              {room !== "BOT" && (
                <button onClick={copy} className="text-white/60 text-[10px] bg-white/10 px-2 py-0.5 rounded-lg active:bg-white/20 font-medium">
                  {copied ? "✅ Gekopieerd!" : "📋 Code"}
                </button>
              )}
            </div>
            <div className={`px-2.5 py-1 rounded-lg text-[10px] font-black shadow ${myTurn ? "bg-green-500 text-black" : "bg-red-500/80 text-white"}`}>
              {myTurn
                ? (lang === "KU" ? "🟢 نۆبەی تۆ" : "🟢 Jouw beurt")
                : `🔴 ${oppName}`
              }
            </div>
          </div>
          ...
          {/* Waiting banner */}
          {waiting && (
            <div className="bg-gradient-to-r from-orange-500 to-yellow-500 text-black font-black text-center py-2.5 text-[11px] flex-shrink-0 shadow-lg">
              <div>⏳ {lang === "KU" ? "کۆدەکە بنێرە بۆ هاوڕێکەت!" : "Stuur de code naar je vriend!"}</div>
              <div className="mt-1 flex items-center justify-center gap-2">
                <span className="bg-black text-white px-3 py-1 rounded-lg font-mono text-base tracking-widest">{room}</span>
                <button onClick={copy} className="bg-white/80 text-black px-2.5 py-1 rounded-lg text-[10px] font-bold active:bg-white shadow">
                  {copied ? "✅" : "📋 Kopieer"}
                </button>
              </div>
            </div>
          ...
          {/* Game over */}
          {gameOver && (
            <div className="bg-gradient-to-r from-yellow-500 via-orange-500 to-red-500 text-black font-black text-center py-3 text-base flex-shrink-0 shadow-lg">
              {gameOver}
              <button onClick={restart} className="ml-3 bg-black text-white px-4 py-1.5 rounded-xl text-xs font-bold active:bg-gray-800 shadow">
                🔄 {lang === "KU" ? "دووبارە" : "Opnieuw"}
              </button>
            </div>
          )}
          ...
          <div className="flex sm:hidden flex-shrink-0">
            <button onClick={() => setTab("board")}
              className={`flex-1 py-2 text-xs font-bold transition-all ${tab === "board" ? "bg-green-900/50 text-green-400 border-b-2 border-green-400" : "text-white/30 bg-black/30"}`}>
              🎮 {lang === "KU" ? "یاری" : "Spel"}
            </button>
            <button onClick={() => { setTab("chat"); setUnread(0); }}
              className={`flex-1 py-2 text-xs font-bold relative transition-all ${tab === "chat" ? "bg-green-900/50 text-green-400 border-b-2 border-green-400" : "text-white/30 bg-black/30"}`}>
              💬 {lang === "KU" ? "چات" : "Chat"}
              {unread > 0 && tab !== "chat" && (
                <span className="absolute top-1 right-[30%] bg-red-500 text-white text-[7px] min-w-[16px] h-4 rounded-full flex items-center justify-center font-bold animate-bounce px-1">
                  {unread}
                </span>
              )}
            </button>
          ...
          <div className="flex-1 flex min-h-0">
            ...
            <div className={`flex-1 flex flex-col min-h-0 ${tab === "chat" ? "hidden sm:flex" : "flex"}`}>
              ...
              <div className="bg-black/40 px-2 py-1.5 flex items-center justify-between flex-shrink-0">
                <div className="flex items-center gap-1.5">
                  ...
                  <div>
                    ...
                    <div className="text-white/30 text-[9px]">{cOpp} {lang === "KU" ? "تابلۆ" : "stenen"}</div>
                  </div>
                </div>
                <div className="flex gap-px overflow-hidden max-w-[55%]">
                  {[...Array(Math.min(cOpp, 14))].map((_, i) => <Back key={i} />)}
                  {cOpp > 14 && <span className="text-white/20 text-[8px] self-center ml-0.5">+{cOpp - 14}</span>}
                </div>
              </div>
              ...
              <div className="flex-1 relative overflow-auto" style={{
                ...
                <div className="absolute top-1.5 right-1.5 bg-black/40 px-2 py-0.5 rounded-lg text-white/50 text-[9px] font-bold backdrop-blur-sm">📦 {cPile}</div>

                <div className="absolute inset-0 flex items-center justify-center p-3">
                  {!board.length ? (
                    <div className="text-white/15 text-center">
                      <div className="text-2xl mb-1.5">☀️</div>
                      <div className="text-xs font-bold">{waiting ? (lang === "KU" ? "چاوەڕوانی..." : "Wacht op vriend...") : (lang === "KU" ? "یاری دەست پێ بکە!" : "Leg eerste steen!")}</div>
                    </div>
                  ) : (
              ...
              <div className="bg-black/50 px-2 py-1.5 flex items-center justify-between flex-shrink-0">
                <div className="flex items-center gap-1.5">
                  ...
                  <div>
                    ...
                    <div className="text-white/30 text-[9px]">{hand.length} {lang === "KU" ? "تابلۆ" : "stenen"}</div>
                  </div>
                </div>
                <div className="flex gap-1.5 items-center">
                  {!canAny() && myTurn && !gameOver && cPile > 0 && (
                    <span className="text-red-400 text-[8px] animate-pulse font-bold">⚠️</span>
                  )}
                  <button onClick={draw} disabled={!myTurn || !!gameOver || cPile === 0}
                    className="bg-blue-600 text-white px-2.5 py-1 rounded-lg text-[10px] font-bold disabled:opacity-25 active:bg-blue-500 shadow transition-colors">
                    📦 {lang === "KU" ? "هەڵگرتن" : "Pak"} ({cPile})
                  </button>
                  {!canAny() && myTurn && !gameOver && cPile === 0 && (
                    <button onClick={pass} className="bg-orange-600 text-white px-2.5 py-1 rounded-lg text-[10px] font-bold active:bg-orange-500 shadow">
                      ⏭️ {lang === "KU" ? "تێپەڕ" : "Pas"}
                    </button>
                  )}
                </div>
            ...
            <div className={`w-full sm:w-52 md:w-56 bg-black/40 flex flex-col border-l border-white/5 ${tab === "board" ? "hidden sm:flex" : "flex"}`}>
              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-1 min-h-0">
                {!msgs.length && <div className="text-white/15 text-[10px] text-center mt-8">{lang === "KU" ? "هیچ پەیامێک نییە" : "Geen berichten"}</div>}
                {msgs.map((m, i) => (
                  <div key={i} className={`p-1.5 rounded-lg text-[10px] leading-snug ${m.sender === "⚙️" ? "bg-blue-500/10 text-blue-300/80 italic" : m.sender.includes("Bot") ? "bg-red-500/10 text-red-300/80" : "bg-white/5 text-white/80"}`}>
              ...
              <div className="flex gap-1 p-2 pt-1">
                <button onClick={() => setShowEm(!showEm)} className={`px-1.5 py-1 rounded-lg text-sm transition-colors ${showEm ? "bg-yellow-500" : "bg-white/10 active:bg-white/20"}`}>😊</button>
                <input value={msg} onChange={e => setMsg(e.target.value)} onKeyDown={e => { if (e.key === "Enter") sendC(msg); }}
                  className="flex-1 bg-white/10 rounded-lg px-2 py-1 text-[11px] text-white outline-none focus:ring-1 focus:ring-yellow-500/50 placeholder-white/20 min-w-0"
