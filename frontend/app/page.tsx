...
const KurdistanSun = ({ size = 200, className = "" }: { size?: number; className?: string }) => (
  <svg width={size} height={size} viewBox="0 0 200 200" className={className}>
    ...
    {[...Array(21)].map((_, i) => {
      ...
      const leftRad = ((angle - 8) * Math.PI) / 180;
      const rightRad = ((angle + 8) * Math.PI) / 180;
      const lx = 100 + Math.cos(leftRad) * 45;
      const ly = 100 + Math.sin(leftRad) * 45;
      const rx = 100 + Math.cos(rightRad) * 45;
      const ry = 100 + Math.sin(rightRad) * 45;
      return (
        <polygon
          key={i}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    s.on("roomCreated", ({ code, hand, playerIndex: pi }: any) => {
      setRoomCode(code); setPlayerHand(hand); setPlayerIndex(pi);
      setWaiting(true); setView("game"); setMode("online");
      setBoard([]); setGameOver(null); setMessages([]);
      addSysMsg(`Kamer ${code} aangemaakt! Wacht op tegenstander...`);
    });

    s.on("roomJoined", ({ code, hand, playerIndex: pi, board: b, turn: t, opponentName: on, opponentHandCount: ohc, pileCount: pc }: any) => {
      setRoomCode(code); setPlayerHand(hand); setPlayerIndex(pi);
      setBoard(b); setMyTurn(t === pi); setOpponentName(on);
      setOpponentHandCount(ohc); setPileCount(pc); setWaiting(false);
      setView("game"); setMode("online"); setGameOver(null); setMessages([]);
      addSysMsg(`Je bent gejoind in kamer ${code}! Tegen: ${on}`);
      setJoinError("");
    });
    ...
    s.on("opponentJoined", ({ opponentName: on, turn: t }: any) => {
      setOpponentName(on); setWaiting(false); setMyTurn(t === 0);
    ...
    s.on("gameUpdate", ({ board: b, hand, turn: t, pileCount: pc, opponentHandCount: ohc, lastPlay, winner, drewTile, opponentDrew, passed }: any) => {
      setBoard(b); setPlayerHand(hand); setPileCount(pc); setOpponentHandCount(ohc);
      setPlayerIndex((currentPI) => { setMyTurn(t === currentPI); return currentPI; });
      if (lastPlay) addSysMsg(`${lastPlay.playerName} speelde [${lastPlay.tile[0]}|${lastPlay.tile[1]}]`);
      if (drewTile) addSysMsg(`📦 Je pakte [${drewTile[0]}|${drewTile[1]}] uit de pot.`);
      if (opponentDrew) addSysMsg(`📦 Tegenstander pakte uit de pot.`);
      if (passed) addSysMsg(`⏭️ ${passed.name} heeft gepast.`);
      if (winner) {
        setGameOver(winner.index === playerIndex ? "🎉 JIJ WINT!" : `💀 ${winner.name} WINT!`);
        addSysMsg(winner.index === playerIndex ? "🎉 Je hebt gewonnen!" : `💀 ${winner.name} heeft gewonnen!`);
      }
    });

    s.on("chatMessage", (msg: Message) => setMessages((p) => [...p, msg]));
    s.on("opponentLeft", () => { addSysMsg("❌ Tegenstander heeft het spel verlaten!"); setGameOver("❌ Tegenstander weg"); });
  ...
  const handleCreateRoom = () => socketRef.current?.emit("createRoom", { playerName });
  ...
  const onlinePlayTile = (index: number) => { if (!myTurn || gameOver) return; socketRef.current?.emit("playTile", { tileIndex: index }); };
  const onlineDrawTile = () => { if (!myTurn || gameOver) return; socketRef.current?.emit("drawTile"); };
  const onlinePass = () => { if (!myTurn || gameOver) return; socketRef.current?.emit("passTurn"); };
  const onlineSendChat = (text: string) => { const t = text.trim(); if (!t) return; socketRef.current?.emit("sendMessage", { text: t }); setCurrentMsg(""); setShowEmojis(false); };
  ...
  const startBotGame = () => {
    ...
    setMode("bot"); setView("game"); setRoomCode("BOT"); setMessages([]);
  ...
  const botPlayTile = (tile: Tile, index: number) => {
    ...
    if (newHand.length === 0) { setGameOver("🎉 JIJ WINT!"); addSysMsg("🎉 Je hebt gewonnen!"); return; }
  ...
  const doBotMove = (currentBoard: BoardEntry[]) => {
    setBotHand((prevBot) => {
      ...
      let playIdx = -1; let playSide: "left" | "right" | null = null;
      ...
      if (playIdx === -1) {
        setPile((pp) => {
          ...
          setTurn("player"); setMyTurn(true); return np;
        });
        return prevBot;
      }
  ...
  const botDrawFromPile = () => {
    ...
    addSysMsg(`📦 Je pakte [${drawn[0]}|${drawn[1]}]`);
  };
  ...
  const botSendChat = (text: string) => {
    ...
    setTimeout(() => {
      const replies = ["Haha 😂","Oke 👍","Ik ga winnen 😈","Nice! 🔥","GG 💯","🤖💪","Hmm...","Goed gespeeld! 👏","Biji Kurdistan! ☀️"];
  ...
  const handlePass = () => {
    if (mode === "bot") { setTurn("opponent"); setMyTurn(false); addSysMsg("⏭️ Je hebt gepast."); setTimeout(() => doBotMove(board), 800); }
    else onlinePass();
  };
  ...
  const DominoTile = ({ value, highlight }: { value: Tile; highlight?: boolean }) => {
    ...
    return (
      <div className={`w-10 h-20 bg-white rounded-lg border-2 flex flex-col items-center justify-around py-1 shadow-md m-0.5 transition-all duration-200 hover:scale-110 cursor-pointer ${highlight ? "border-yellow-400 ring-2 ring-yellow-400 shadow-yellow-400/50 shadow-lg" : "border-gray-400"}`}>
        {[0, 1].map((side) => (
    ...
    <main className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="fixed inset-0 z-0">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <KurdistanSun size={view === "menu" ? 300 : 400} className="opacity-30" />
        </div>
      </div>

      {/* Overlay voor game view (donkerder zodat je het bord kunt zien) */}
      {view === "game" && (
      ...
      <div className="absolute top-4 right-4 flex gap-2 z-50 items-center">
        <div className={`w-2 h-2 rounded-full ${connected ? "bg-green-400" : "bg-red-500"} shadow-lg`} title={connected ? "Verbonden" : "Geen verbinding"} />
      ...
      {view === "menu" ? (
        <div className="flex flex-col items-center z-10 w-full max-w-xs p-4">
          ...
          <div className="relative mb-4">
            <KurdistanSun size={150} className="absolute -top-10 left-1/2 -translate-x-1/2 opacity-80" />
            <h1 className="text-7xl font-black text-white italic drop-shadow-[0_4px_8px_rgba(0,0,0,0.5)] relative z-10 tracking-tight">
              DOMINO
            </h1>
          </div>

          <p className="text-white/90 text-sm mb-6 font-bold bg-black/20 px-4 py-1 rounded-full backdrop-blur-sm">
            🎲 28 {lang === "KU" ? "تابلۆ" : "stenen"} • 7 {lang === "KU" ? "بۆ هەر یاریزان" : "per speler"}
          </p>

          {/* Naam */}
          <div className="w-full mb-4">
            <label className="text-white/80 text-xs font-bold mb-1 block">{lang === "KU" ? "ناوی تۆ" : "Je naam"}</label>
            <input
              type="text"
              placeholder={lang === "KU" ? "ناوی تۆ..." : "Je naam..."}
              value={playerName}
              onChange={(e) => setPlayerName(e.target.value || "Speler")}
              className="w-full p-3 rounded-xl text-center font-bold text-black bg-white/90 backdrop-blur-sm shadow-lg border-2 border-yellow-400 focus:outline-none focus:ring-2 focus:ring-yellow-500"
            />
          </div>

          <button
            onClick={startBotGame}
            className="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white font-black py-4 rounded-2xl border-b-[6px] border-blue-900 mb-3 active:translate-y-1 hover:from-blue-500 hover:to-blue-600 transition-all shadow-xl text-lg"
          >
          ...
          <button
            ...
            className="w-full bg-gradient-to-r from-green-600 to-green-700 text-white font-black py-4 rounded-2xl border-b-[6px] border-green-900 mb-3 active:translate-y-1 hover:from-green-500 hover:to-green-600 transition-all shadow-xl text-lg"
          >
          ...
          <div className="bg-gradient-to-r from-purple-600 to-purple-700 p-4 rounded-2xl border-b-[6px] border-purple-900 w-full shadow-xl">
            <input
              ...
              className="w-full p-3 rounded-xl text-center font-bold mb-2 uppercase text-black bg-white/90 border-2 border-purple-300 focus:outline-none focus:ring-2 focus:ring-purple-400"
              maxLength={5}
            />
            {joinError && <p className="text-red-300 text-xs text-center mb-2 font-bold">⚠️ {joinError}</p>}
            <button
              onClick={handleJoinRoom}
              className="w-full text-white font-black text-sm py-3 bg-purple-900/50 rounded-xl hover:bg-purple-800/50 transition-colors"
            >
          ...
          {/* Kurdistan credit */}
          <p className="text-white/50 text-xs mt-6 font-bold">☀️ Biji Kurdistan</p>
        </div>
      ) : (
        <div className="w-full h-screen flex flex-col p-2 md:p-4 max-w-6xl z-10">
          {/* Top bar */}
          <div className="flex justify-between items-center mb-2 flex-wrap gap-2">
            <button onClick={() => { setView("menu"); setWaiting(false); }} className="bg-black/40 backdrop-blur-sm text-white px-4 py-2 rounded-xl text-sm hover:bg-black/60 transition-colors font-bold border border-white/10">
              ↩ {lang === "KU" ? "گەڕانەوە" : "Menu"}
            </button>

            <div className="flex items-center gap-2">
              <div className="bg-gradient-to-r from-yellow-500 to-orange-500 text-black px-4 py-2 rounded-xl font-black text-sm shadow-lg">
                🏠 {roomCode}
              </div>
              {roomCode !== "BOT" && (
              ...
              <div className={`px-3 py-2 rounded-xl text-xs font-black ${myTurn ? "bg-green-500 text-black" : "bg-red-500/80 text-white"}`}>
                {myTurn ? "🟢 Jouw beurt" : `🔴 ${mode === "bot" ? "Bot" : opponentName || "..."}`}
              </div>
            </div>
          ...
          {/* Waiting */}
          {waiting && (
            <div className="bg-gradient-to-r from-orange-500 to-yellow-500 text-black font-black text-center py-3 rounded-2xl mb-2 animate-pulse shadow-lg">
              ⏳ {lang === "KU" ? "چاوەڕوانی یاریزانی تر..." : "Wacht op tegenstander..."} 
              <span className="bg-black text-white px-3 py-1 rounded-lg ml-2 font-mono">{roomCode}</span>
              <button onClick={copyCode} className="ml-2 bg-white text-black px-2 py-1 rounded text-xs font-bold">{copied ? "✅" : "📋 Kopieer"}</button>
            </div>
          )}
          ...
          {gameOver && (
            <div className="bg-gradient-to-r from-yellow-500 via-orange-500 to-red-500 text-black font-black text-center py-4 rounded-2xl mb-2 text-xl shadow-2xl">
              {gameOver}
              {mode === "bot" && (
                <button onClick={startBotGame} className="ml-4 bg-black text-white px-5 py-2 rounded-xl text-sm font-bold hover:bg-gray-800 transition-colors">🔄 {lang === "KU" ? "دووبارە" : "Opnieuw"}</button>
              )}
            </div>
          )}

          <div className="flex gap-2 flex-1 min-h-0">
            {/* Speelveld met Kurdistan thema */}
            <div className="flex-1 rounded-[20px] bg-black/30 backdrop-blur-sm flex flex-col items-center justify-center p-2 overflow-auto relative border-2 border-yellow-500/30">
              {/* Kurdistan zon watermark op bord */}
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <KurdistanSun size={200} className="opacity-5" />
              </div>

              {/* Vlag strepen subtiel op bord */}
                ...
                <div className="text-white/50 text-sm text-center relative z-10">
                  ...
                  {waiting ? (
                    ...
                    <>
                      ...
                      <br />
                      <span className="text-xs text-white/30">{lang === "KU" ? "کلیک لەسەر تابلۆیەک لە خوارەوە" : "Klik op een tegel hieronder"}</span>
                    </>
                  )}
                </div>
              ) : (
            ...
            <div className="w-56 md:w-64 bg-black/40 backdrop-blur-sm rounded-[20px] flex flex-col p-3 border border-yellow-500/20">
              ...
              {showEmojis && (
                <div className="grid grid-cols-6 gap-1 mb-2 bg-black/60 p-2 rounded-lg max-h-28 overflow-y-auto border border-white/10">
                  {emojis.map((e) => (
                    <button key={e} onClick={() => setCurrentMsg((p) => p + e)} className="hover:bg-white/20 rounded p-1 text-sm transition-colors hover:scale-125">{e}</button>
                  ))}
                </div>
              )}
              ...
              <div className="flex gap-1">
                ...
                <input
                  ...
                  className="flex-1 bg-white/10 rounded-lg p-2 text-xs text-white outline-none focus:ring-1 focus:ring-yellow-500 placeholder-white/30"
                  placeholder={lang === "KU" ? "بنووسە..." : "Typ..."}
                />
                <button onClick={() => handleSendChat(currentMsg)} className="bg-gradient-to-r from-green-600 to-green-700 px-3 rounded-lg text-xs text-white font-bold hover:from-green-500 hover:to-green-600 transition-all">➤</button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-black/40 backdrop-blur-sm p-3 rounded-[20px] border-t-4 border-yellow-500 mt-2">
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="flex gap-2 items-center">
                {!canPlayerPlayAny() && myTurn && !gameOver && (mode === "bot" ? pile.length > 0 : pileCount > 0) && (
                  <span className="text-red-400 text-xs animate-pulse font-bold">⚠️ {lang === "KU" ? "ناتوانی!" : "Geen opties!"}</span>
                )}
                <button
                  onClick={handleDraw}
                  disabled={!myTurn || !!gameOver || (mode === "bot" ? pile.length === 0 : pileCount === 0)}
                  className="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-4 py-2 rounded-xl text-xs font-bold hover:from-blue-500 hover:to-blue-600 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-lg"
                >
                  📦 {lang === "KU" ? "هەڵگرتن" : "Pak"} ({mode === "bot" ? pile.length : pileCount})
                </button>
                {!canPlayerPlayAny() && myTurn && !gameOver && (mode === "bot" ? pile.length === 0 : pileCount === 0) && (
                  <button onClick={handlePass} className="bg-gradient-to-r from-orange-600 to-orange-700 text-white px-4 py-2 rounded-xl text-xs font-bold hover:from-orange-500 hover:to-orange-600 transition-all shadow-lg">
            ...
            <div className="flex justify-center gap-1 overflow-x-auto pb-1">
              {playerHand.map((t, i) => {
                ...
                return (
                  <div key={i} onClick={() => handlePlay(t, i)} className={`transition-all duration-200 ${!myTurn || gameOver ? "opacity-30 pointer-events-none" : playable ? "hover:-translate-y-3 hover:shadow-xl" : "opacity-40"}`} title={playable ? `[${t[0]}|${t[1]}]` : "Past niet"}>
